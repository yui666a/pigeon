...
pub fn get_by_mail_id(conn: &Connection, mail_id: &str) -> Result<Vec<Attachment>, AppError> {
    let mut stmt = conn.prepare(
        "SELECT id, mail_id, filename, mime_type, size, file_path
...
pub fn delete_by_mail_id(conn: &Connection, mail_id: &str) -> Result<(), AppError> {
    conn.execute("DELETE FROM attachments WHERE mail_id = ?1", params![mail_id])?;
    Ok(())
}
...
mod tests {
    ...
    fn test_get_by_mail_id_orders_by_filename() {
        ...
        insert_attachment(&conn, "m1", "b.txt", "text/plain", 1, "/c/b.txt").unwrap();
        insert_attachment(&conn, "m1", "a.txt", "text/plain", 1, "/c/a.txt").unwrap();
    ...
    fn test_get_by_id() {
        ...
        insert_test_mail(&conn, "m1", "With attachment");
        let att =
            insert_attachment(&conn, "m1", "a.png", "image/png", 5, "/c/a.png").unwrap();
