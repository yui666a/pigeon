...
///
/// Stronghold の初期化コストは `iota_stronghold::KeyProvider` の鍵導出（意図的に
/// 重い KDF）が支配的で、保管件数に依存せず固定で数十秒かかる。これを
/// `tauri::Builder` 構築前に実行すると、ウィンドウが存在しない状態で待つことになり
/// ユーザーには完全な無反応に見える。SecureStore は起動時には一切使われず、
/// 実際の利用は `sync_account` の資格情報解決などユーザー操作時であるため、
/// 最初に秘密情報を必要とする操作の時点まで初期化を遅延させる。
///
/// 初期化に失敗した場合は panic せず `AppError` を呼び出し元のコマンドへ伝播する
/// （ADR 0001 のサイレントフォールバック禁止は「失敗を隠すな」であって
/// 「panic しろ」ではない。既にウィンドウが出ている時点で落とすのは体験上最悪）。
/// 失敗は記憶されないため、次回アクセス時に再試行される。
...
impl SecureStoreState {
    ...
    /// 初期化に失敗した場合はエラーを返し、実体は未初期化のまま残す
    /// （次回アクセスで再試行される）。
...
mod tests {
    ...
    fn test_secure_store_shares_the_same_instance() {
        ...
        state.get().unwrap().insert("k", b"v").unwrap();
        assert_eq!(
            state.get().unwrap().get("k").unwrap().as_deref(),
            Some(b"v".as_ref()),
    ...
    fn test_secure_store_init_failure_returns_error_instead_of_panicking() {
        ...
        // 呼び出し元のコマンドへ伝播すること（ADR 0001 / agent.md）
        let state =
    ...
    fn test_secure_store_concurrent_access_initializes_once() {
        ...
        for handle in handles {
            handle.join().expect("スレッドは panic しない").unwrap();
        }
