...
OAuth クライアント ID/シークレットの供給（2026-07-16 追記）: 開発時は `.env`（`dotenvy` が起動時に環境変数へ流し込む。`src-tauri/src/env_config.rs` が cwd 非依存で探索）で渡す。配布ビルドには `.env` が同梱されないため、リリース CI（`.github/workflows/release.yml`、release environment の `PIGEON_GOOGLE_*` secrets）がビルド時に環境変数として渡し、`src-tauri/src/mail_sync/oauth.rs` の `option_env!` がバイナリへ焼き込む（`build.rs` の `rerun-if-env-changed` でビルドキャッシュを無効化）。解決の優先順位は「実行時の環境変数 → ビルド時埋め込み」。Google の Desktop OAuth クライアントはネイティブアプリ配布において client secret を機密として保証できない（バイナリから抽出可能）ことを前提とする公開情報扱いであり、PKCE と loopback リダイレクトで安全性を担保する。
