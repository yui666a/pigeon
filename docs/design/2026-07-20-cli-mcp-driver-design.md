...
DB は単一の `Mutex<Connection>` であり、Stronghold のスナップショットはファイルロックを取る。いずれも GUI プロセスと共有できない。

**GUI 起動中に CLI を実行した場合はエラーで終了する**方針とする。CLI 起動時にロック取得を試み、失敗したら「Pigeon が起動中です」と明示して終了する。取得失敗を曖昧なエラーのまま伝えない。
