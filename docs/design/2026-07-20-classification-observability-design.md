...
## 4.1 プロンプトのキャリブレーション（追記: 2026-07-20）

観測の土台を入れたうえで、確信度が高値に偏る原因のうちプロンプト側の2点を修正した。

**原因1: 確信度の定義がなかった。** 修正前は「0.0〜1.0の浮動小数」という値域宣言1行のみで、値の意味が定義されていなかった。「confidence は正解する確率であり、0.9 なら10回に1回外すはず」という頻度論的な定義と、帯ごとの基準（0.9以上は送信者と件名の両方が一致するときのみ、0.4〜0.7が大半のメールで期待される帯）を追加した。

**原因2: 「自信のない assign」という選択肢がなかった。** `unclassified` を "only when ... too ambiguous" と強く抑制していたため、迷ったときの逃げ道が create か unclassified にしかなく、assign を選んだ時点で「確信している」文脈に入っていた。「候補があるなら低確信の assign を unclassified より優先せよ。低確信の割り当てはユーザーに確認を求めるので、外れても安い」という指示を追加した。

あわせてフォーマット例に低確信 assign（0.55）の例を追加した。修正前は action ごとに確信度が1つずつ固定で紐付いており、「assign なら高い」というカテゴリ連想を助長していた。

**この修正で改善する保証はない。** 小〜中規模モデルの自己申告確信度は構造的に粗く、プロンプトで矯正できるとは限らない。効果は `classification_log` の蓄積で判定する。改善しなかった場合は、確信度そのものを信頼するのをやめ、閾値の再設定や structured output の導入を検討する。
