...
mod tests {
    ...
    fn test_v18_creates_tables_and_vec0_works() {
        ...
        conn.execute(
            ...
            params![vec![0.0f32; 1024].as_bytes()],
        )
        .unwrap();
...
- Modify: `src-tauri/src/classifier/mod.rs`（`build_http_client` を `pub(crate)` に。既に可視なら変更不要）
...
**Interfaces:**
- Consumes: `crate::classifier::build_http_client()`（30秒タイムアウトの reqwest Client）、`AppError::{OllamaConnection, InvalidLlmResponse}`、db::settings の `get_or_default`
- Produces:
...
impl OllamaEmbedder {
    ...
    pub fn from_settings(conn: &rusqlite::Connection) -> Result<Self, AppError> {
        ...
        Ok(Self::new(endpoint, model, dimensions))
    }
}
...
settings への追記: `AppError` 型・関数名は実ファイルに合わせること。`classifier/mod.rs` の `build_http_client` が private なら `pub(crate)` に変更。`async_trait` が Cargo.toml に無ければ追加（classifier が既に使っていれば不要）。
...
```bash
git add src-tauri/src/embedding src-tauri/src/lib.rs src-tauri/src/classifier/mod.rs src-tauri/src/commands/settings_commands.rs src/types/settings.ts src/api/settingsApi.ts src-tauri/Cargo.toml src-tauri/Cargo.lock
...
- Produces:
  - `pub async fn run_embedding_pass(db: &tokio::sync::Mutex<rusqlite::Connection>, embedder: &dyn Embedder, doc_prefix: &str, on_progress: &mut (dyn FnMut(u64, u64) + Send)) -> Result<u64, AppError>`
    - 戻り値 = 埋め込んだチャンク数。Ollama 接続エラー時は**エラーにせずそこで打ち切って Ok を返す**（キューに残す。次回パスで再開）
    - ※ DB State の実型（`Mutex<Connection>` が std か tokio か、ラッパ struct か）は lib.rs / Ctx の実物に合わせて調整すること。ロックを await をまたいで保持しない構造が要件
...
`worker.rs` のテスト（DB は `setup_db` の in-memory を Mutex に包む。FakeEmbedder 使用）:
...
mod tests {
    ...
    fn db() -> tokio::sync::Mutex<rusqlite::Connection> {
        tokio::sync::Mutex::new(setup_db())
    }
...
//! DB ロックは await をまたいで保持しない（1ステップごとに取得・解放）。
...
pub async fn run_embedding_pass(
    ...
    loop {
        let batch = {
            let conn = db.lock().await;
            chunks::mails_without_chunks(&conn, CHUNKING_BATCH)?
        };
        if batch.is_empty() {
            break;
        }
    ...
    let total = {
        let conn = db.lock().await;
        chunks::pending_totals(&conn)?.1
    };
    let mut done: u64 = 0;
    loop {
        let batch = {
            let conn = db.lock().await;
            chunks::pending_chunks(&conn, EMBED_BATCH)?
        };
        if batch.is_empty() {
            break;
        }
...
4. DB State の実型が `tokio::sync::Mutex` でない場合（std Mutex 等）は、worker の引数型を実物に合わせて調整する（ロックを await をまたいで保持しない構造は維持）
