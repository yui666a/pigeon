...
- **同一アカウントの同期は多重実行しない**（実装後の追補）: スレッド一覧は表示のたびに `sync_account` を呼ぶため、画面遷移や React 開発モードの二重 effect で同期が並行し得る。並行すると全員が同期前の `max_uid` を見て同じメールを多重取り込みする（実際に3重取り込みが発生した）。対策は3層: (1) バックエンドのアカウント単位ロック（`SyncLocks`。進行中なら即 0 を返す）、(2) フロント `syncAccount` の `syncing` 中スキップ、(3) 最終防衛線として `mails` に `UNIQUE(account_id, folder, uid)`（migrate_v6。既存の重複はマイグレーションで掃除）+ `insert_mail` の `INSERT OR IGNORE` 化。OR REPLACE は UNIQUE 衝突時に既存行を消し案件割り当てを CASCADE で失うため使わない
...
| 同期の多重起動（画面遷移・開発モードの二重effect） | アカウント単位ロックで2本目以降は即 0 を返す。万一すり抜けても `UNIQUE(account_id, folder, uid)` + `INSERT OR IGNORE` が重複挿入を防ぐ |
