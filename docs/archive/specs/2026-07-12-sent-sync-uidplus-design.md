...
> **追記（2026-07-13 リファクタリング）**: 本書で `commands/mail_commands.rs` 内の関数として
> 記述している同期オーケストレーション（`sync_account_inner` / `sync_folder_into` /
> `sync_sent_folder` / `MergeStrategy`）は、`mail_sync/sync_service.rs` へ移動した。
> コマンド層は SyncLocks 制御・進捗イベント emit・資格情報解決の注入のみを担う。
> 同期の流れ・watermark・マージ方式の設計自体は本書のとおり。
