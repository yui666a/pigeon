...
- フロント: `@tauri-apps/plugin-dialog` の `open({ multiple: true })` でパスを取得し、
  ...
  `open` はパスのみを返すため、表示用の `size` は Rust の `stat_file` command で取得する
  （`@tauri-apps/plugin-fs` は導入しない。fs capability を広げず、既存の
  「パスを Rust へ渡す」方式に揃えるため）
...
- **下書き保存はプレーンに落として保存**する。`drafts` テーブルのスキーマは変えない
  （`body_text` のみ）。リッチ本文は保存時にフロントの `htmlToPlain`（DOMParser 版）で
  プレーン化して保存し、復元時はプレーンモードで開く。この `htmlToPlain` は
  Rust の `html_to_plain`（送信時 plain フォールバック）と変換規則を揃える
  （ブロック終端＝単一改行、`<li>`＝先頭 `- `、連続空行は1つに圧縮）。
  **添付は下書きに保存しない**（パスは端末依存で永続化に不向き）
