...
Gmail に OAuth 2.0 で接続できるようにする。カスタムURLスキーム（`com.haiso666.pigeon://oauth/callback`）でブラウザからの認可コードを受け取り、Stronghold にトークンを保存する。将来の iOS / 他プロバイダ対応を見据えた設計。
...
```
 ...
 8. ユーザーがブラウザでログイン・許可
 9. ブラウザが com.haiso666.pigeon://oauth/callback?code=xxx&state=yyy にリダイレクト
...
| プラグイン | 用途 |
|-----------|------|
| `tauri-plugin-deep-link` | カスタムURLスキーム `com.haiso666.pigeon://` の登録とコールバック受信 |
| `tauri-plugin-stronghold` | OAuth トークン / パスワードのセキュア保存 |

### Deep Link スキーム

`com.haiso666.pigeon://oauth/callback` を使用する。リバースドメイン形式にすることで他アプリとの衝突リスクを軽減する。
