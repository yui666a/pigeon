...
Archive フォルダを一覧表示する導線は無い（`get_threads` は INBOX 表示にのみ
使われる）。ただし `get_threads_by_project`（案件ビュー）と `search_mails`
（検索）は folder で絞り込まないため、アーカイブ済みメールは**案件ビューと
検索結果には表示され続ける**。これが仕様であり（案件の文脈・検索性を維持する
のがアーカイブの価値）、アーカイブ解除の操作もこの導線から行う。

### アーカイブ解除（unarchive_mail）

`mails.folder` が `'Archive'` のメールを `'INBOX'` に戻す。folder が
`'Archive'` 以外のメールに対しては `Validation` エラーを返す。

**v1 ではサーバー反映を行わない（ローカルのみ）**。理由は UID の追跡不能:

- アーカイブ時に COPY 先の新 UID を取得・保存していない（UIDPLUS の
  COPYUID 応答を読んでいない）
- google: アーカイブ済みメールのサーバー実体は All Mail にあるが、ローカルに
  記録された `uid` は INBOX 時代の UID であり、All Mail 上では別 UID
- other: `archive_folder` へ COPY した後の UID が不明

UID が特定できないままサーバーを操作すると別メールを操作する危険があるため、
v1 では `mails::update_folder(mail_id, 'INBOX')` によるローカル更新のみとする。
結果として**サーバー側はアーカイブ済みのまま残る**（次回同期でも INBOX には
戻らない）。サーバー反映は、アーカイブ時に COPYUID を保存する改修とセットで
将来対応する。

フロントエンドは folder を `'INBOX'` に更新するのみで、表示状態からメールを
除去しない（アーカイブ済みメールが見えるのは案件ビュー・検索であり、解除後も
同じ場所に表示され続けるのが自然なため）。
...
- `delete_mail(account_id, mail_id)` / `archive_mail(account_id, mail_id)` /
  `unarchive_mail(account_id, mail_id)`
...
- `MailActions.tsx`: 「アーカイブ」「削除」ボタンを追加。削除は `window.confirm` で確認。
  `mail.folder === 'Archive'` のときは「アーカイブ」の代わりに「アーカイブ解除」を表示する
...
- **アーカイブ解除（Unarchive）はローカルのみ**: サーバー側はアーカイブ済みの
  まま残る（上記「アーカイブ解除」の UID 追跡不能の理由による）
