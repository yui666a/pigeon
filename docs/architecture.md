# アーキテクチャ概要

## 構成

モノリシック構成。Tauri 2 単一プロセス内でReact UI + Rustバックエンドが動作する。

```
┌─ Pigeon (Tauri 2 App) ────────────────────────────────────┐
│                                                             │
│  ┌─ React (WebView) ────────────────────────────────────┐   │
│  │  Zustand / Tailwind CSS v4                            │   │
│  │                                                       │   │
│  │  ┌──────────┬──────────────────────┬────────────────┐ │   │
│  │  │ Sidebar  │ ThreadList           │ MailView       │ │   │
│  │  │          │ Unclassified         │                │ │   │
│  │  │AccountLst│ ThreadItem[]         │ MailHeader /   │ │   │
│  │  │AccountFrm│                      │ Body           │ │   │
│  │  │ProjectTre│                      │                │ │   │
│  │  │ProjectFrm│                      │                │ │   │
│  │  └──────────┴──────────────────────┴────────────────┘ │   │
│  └────────────────────┬──────────────────────────────────┘   │
│                       │ invoke()                             │
│  ┌─ Rust Backend ─────▼──────────────────────────────────┐   │
│  │                                                       │   │
│  │  ┌──────────┐ ┌───────────┐ ┌───────────────────────┐│   │
│  │  │MailSync  │ │Classifier │ │SearchEngine           ││   │
│  │  │  ✅ 実装済│ │  ✅ 実装済 │ │  未実装               ││   │
│  │  └────┬─────┘ └─────┬─────┘ └──────────┬────────────┘│   │
│  │       │             │                  │             │   │
│  │  ┌────▼─────────────▼──────────────────▼───────────┐ │   │
│  │  │                 SQLite DB  ✅ 実装済              │ │   │
│  │  └─────────────────────────────────────────────────┘ │   │
│  └───────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```

## Rustモジュール

| モジュール | 責務 | 状態 |
|-----------|------|------|
| `models` | Account / Mail / Thread / Project / Classifier 構造体、AccountProvider、AppError | ✅ Phase 2 実装済 |
| `db` | SQLiteスキーマ管理・accounts/mails/projects/assignments CRUD・スレッド構築 | ✅ Phase 2 実装済（V3マイグレーション済） |
| `mail_sync` | IMAP接続・差分同期（初回は直近20件）・MIMEパース・OAuth 2.0フロー | ✅ Phase 2 実装済 |
| `commands` | Tauri commandsとしてReactに公開するAPI群（mail/account/auth/project/classify） | ✅ Phase 2 実装済 |
| `secure_store` | Stronghold によるトークン/パスワードの暗号化保存 | ✅ OAuth で追加済 |
| `classifier` | LLM抽象レイヤー（Ollama / Claude API）・プロジェクト分類・確信度スコア | ✅ Phase 2 実装済 |
| `search` | SQLite FTS5を使った全文検索・案件横断検索 | Phase 4 予定 |

## Tauri Commands API

### アカウント・メール管理（Phase 1）

| コマンド | 引数 | 戻り値 | 用途 |
|---------|------|--------|------|
| `create_account` | CreateAccountRequest | Account | アカウント作成 |
| `get_accounts` | — | Account[] | アカウント一覧 |
| `remove_account` | id | — | アカウント削除 |
| `sync_account` | account_id | u32 (取得数) | IMAP同期（認証情報はバックエンドで解決） |
| `get_threads` | account_id, folder | Thread[] | スレッド一覧取得 |
| `start_oauth` | provider | — | OAuth 認可フロー開始（ブラウザを開く） |
| `handle_oauth_callback` | url | Account | OAuth コールバック処理・アカウント保存 |

### プロジェクト管理（Phase 2）

| コマンド | 引数 | 戻り値 | 用途 |
|---------|------|--------|------|
| `get_projects` | account_id | Project[] | プロジェクト一覧取得 |
| `create_project` | account_id, name, description?, color? | Project | プロジェクト作成 |
| `update_project` | id, name?, description?, color? | — | プロジェクト更新 |
| `archive_project` | id | — | プロジェクトアーカイブ |
| `delete_project` | id | — | プロジェクト削除 |

### AI分類（Phase 2）

| コマンド | 引数 | 戻り値 | 用途 |
|---------|------|--------|------|
| `classify_mail` | mail_id | ClassifyResponse | 単一メールのAI分類 |
| `classify_unassigned` | account_id | — | 未分類メール一括分類（イベント通知） |
| `cancel_classification` | — | — | 分類キャンセル |
| `approve_classification` | mail_id, project_id | — | AI分類承認 |
| `approve_new_project` | mail_id, project_name, description? | Project | 新規プロジェクト提案を承認 |
| `reject_classification` | mail_id | — | AI分類却下 |
| `get_unclassified_mails` | account_id | Mail[] | 未分類メール一覧取得 |
| `get_mails_by_project` | project_id | Mail[] | プロジェクト別メール取得 |

## データフロー

### メール受信（Phase 1 実装済）

```
sync_account コマンド呼び出し (account_id のみ)
  → accounts テーブルからアカウント情報取得
  → provider で分岐:
    Google: Stronghold から OAuth トークン取得 → 必要ならリフレッシュ → XOAUTH2 で IMAP接続
    Other:  Stronghold からパスワード取得 → PLAIN で IMAP接続
  → UID差分フェッチ (since_uid 以降, 初回は直近20件)
  → MIMEパース (mail-parser)
  → SQLite保存 (INSERT OR REPLACE)
  → get_threads でスレッド構築 (Union-Find + 件名フォールバック)
  → フロントエンドに返却
```

### AI分類フロー（Phase 2 実装済）

```
classify_unassigned コマンド呼び出し (account_id)
  → 未分類メール一覧取得 (assignments テーブル非存在メール)
  → Ollama ヘルスチェック
  → 各メールに対してループ:
    → プロジェクト一覧を再取得（新規作成を反映）
    → MailSummary 生成（件名・送信者・本文冒頭300文字）
    → LLM 分類実行 → ClassifyResult 取得
    → 結果に応じて分岐:
        Assign（確信度 >= 閾値）: mail_project_assignments に保存
        Create: PendingClassifications に追加（ユーザー確認待ち）
        Unclassified: スキップ
    → classify-progress イベント送信
  → classify-complete イベント送信
  → フロントエンドで自動リフレッシュ（プロジェクト一覧・未分類一覧）
```

### 手動修正 → フィードバック（Phase 3 予定）

```
ユーザーがD&Dで案件を変更
  → mail_project_assignments 更新
  → correction_log に記録
  → 次回LLM呼び出し時にプロンプトに含める
```

## 主要クレート

| 用途 | クレート | 状態 |
|------|---------|------|
| IMAP | `async-imap` 0.11 + `async-native-tls` | 使用中 |
| 非同期トレイト | `async-trait` | 使用中（LLM抽象化） |
| MIMEパース | `mail-parser` | 使用中 |
| DB | `rusqlite` (bundled) | 使用中 |
| JSON | `serde` / `serde_json` | 使用中 |
| 非同期 | `tokio` | 使用中 |
| エラー | `thiserror` | 使用中 |
| ID生成 | `uuid` | 使用中 |
| 日時 | `chrono` | 使用中 |
| HTTP | `reqwest` | 使用中（OAuth トークン交換・Ollama API） |
| セキュアストレージ | `iota-stronghold` | 使用中（トークン/パスワード暗号化保存） |
| Deep Link | `tauri-plugin-deep-link` | 使用中（OAuth コールバック受信） |
| PKCE/暗号 | `sha2` + `rand` + `base64` | 使用中（OAuth PKCE フロー） |
| SMTP | `lettre` | Phase 4 で追加予定 |

## 主要フロントエンドライブラリ

| 用途 | ライブラリ | 状態 |
|------|-----------|------|
| UIフレームワーク | React 19 | 使用中 |
| 状態管理 | Zustand 5 | 使用中 |
| スタイル | Tailwind CSS v4 | 使用中 |
| テスト | Vitest 4 + React Testing Library | 使用中 |
| D&D | React DnD | Phase 3 で追加予定 |
| リッチテキスト | TipTap | Phase 4 で追加予定 |

## フロントエンドストア

| ストア | 責務 | 状態 |
|-------|------|------|
| `accountStore` | アカウント一覧・選択・OAuth フロー管理 | ✅ Phase 2 実装済 |
| `mailStore` | スレッド一覧・メール同期・選択状態 | ✅ Phase 2 実装済 |
| `projectStore` | プロジェクト一覧・CRUD・選択状態 | ✅ Phase 2 実装済 |
| `classifyStore` | AI分類状態・進捗・結果・イベントリスナー | ✅ Phase 2 実装済 |

## DBスキーマバージョン

| バージョン | 追加内容 |
|-----------|---------|
| V1 | accounts / mails テーブル |
| V2 | settings テーブル |
| V3 | projects / mail_project_assignments テーブル |
