...
詳細な設計は `docs/design/2026-04-12-pigeon-design.md` を参照すること。
...
- コードを変更する前に `docs/design/` 配下の設計書と `docs/adr/` の該当ADRを確認すること
- 横断的な設計判断（LLM抽象化、クラウド送信境界、機密情報保管、AI-Nativeアーキ、メール同期規約）は `docs/adr/` を正とすること
...
- 大きな変更は Stacked PR（依存する PR を数珠つなぎ）で分割してよい
- Stacked PR の場合は依存関係（親 PR / 子 PR）を説明に明記する
- コミットは作業完了後の 1 コミットではなく、意味のある粒度で分ける（1コミット = 1意図）
...
このプロダクトは個人開発であり、既存挙動の形式的な互換維持よりも設計の健全性を優先できる。

- 症状を一時的に抑えるだけのパッチではなく、原因に対する修正を優先する
- 修正時は周辺設計との整合性を取り、将来の変更コストが下がる形を選ぶ
- その場しのぎの分岐・例外処理・フラグ追加で逃げない
- 必要であれば、関連モジュールを含めて責務分離やデータ構造を見直す
- 「最小工数でとりあえず直す」より「再発しにくい設計」を優先する
...
- LLMへ送信するデータは、件名・送信者・本文冒頭1000文字、および案件ディレクトリ連携のコンテキスト（`docs/design/2026-07-09-project-directory-context-design.md` の送信可否ポリシーに従う）に限定する。クラウドLLMへのファイル由来データの送信はユーザーが明示的に許可したものに限る
